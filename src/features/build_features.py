...
def _map_binary_series(s: pd.Series) -> pd.Series:
    ...
    """
    # Handle NaN: if series is string type, "nan" strings might be actual NaN values
    # Create a mask for actual NaN values
    if s.dtype == 'object':
        # Check if we have string "nan" that should be treated as NaN
        nan_mask = (s == 'nan') | s.isna()
        # Get unique values excluding NaN and "nan" strings
        vals = list(pd.Series(s[~nan_mask].unique()).astype(str))
    else:
    ...
    if valset == {"Yes", "No"}:
        result = s.map({"No": 0, "Yes": 1})
        # Preserve NaN values (treat "nan" strings as NaN)
        if s.dtype == 'object':
            result = result.where(s != 'nan', pd.NA)
        return result.astype("Int64")
        
    # Gender mapping (demographic feature)
    if valset == {"Male", "Female"}:
        result = s.map({"Female": 0, "Male": 1})
        # Preserve NaN values (treat "nan" strings as NaN)
        if s.dtype == 'object':
            result = result.where(s != 'nan', pd.NA)
        return result.astype("Int64")
...
def build_features(df: pd.DataFrame, target_col: str = "Churn") -> pd.DataFrame:
    ...
    for c in binary_cols:
        ...
        # Convert to string for mapping (handles NaN -> "nan" string conversion)
    ...
    # === STEP 6: Convert Boolean Columns (including one-hot encoded) ===
    # XGBoost requires integer inputs, not boolean
    # This handles boolean columns created by one-hot encoding
    bool_cols = df.select_dtypes(include=["bool"]).columns.tolist()
    if bool_cols:
        df[bool_cols] = df[bool_cols].astype(int)
        print(f"   Converted {len(bool_cols)} boolean columns to int (from one-hot encoding)")

    # === STEP 7: Data Type Cleanup ===
